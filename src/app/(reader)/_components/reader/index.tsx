...
export default function Reader({ images, chapterData }: ReaderProps) {
  ...
  const currentSpreadIdx = useMemo(
    () =>
      Math.max(
        0,
        spreads.findIndex((s) => s.includes(currentIndex)),
      ),
  ...
  const { prevChapterId, nextChapterId } = useMemo(() => {
    ...
    const next =
      chapterAggregate[volIdx]?.chapters[chIdx - 1]?.id ??
      chapterAggregate[volIdx - 1]?.chapters.at(-1)?.id;
    return { prevChapterId: prev, nextChapterId: next };
  }, [chapterAggregate, chapterExists, chapterData.id]);

  const goPrevChapter = useCallback(() => {
    if (prevChapterId) {
      toast.info("Switching chapter...", { duration: 3000, closeButton: false, icon: <Spinner/> });
      router.push(`/chapter/${prevChapterId}`);
    } else toast.warning("This is the first chapter!");
  ...
  const goNextChapter = useCallback(() => {
    ...
    } else toast.warning("This is the latest chapter!");
  ...
  const goPrevPage = useCallback(() => {
    if (mode === "double") {
      if (currentSpreadIdx > 0)
        setCurrentIndex(spreads[currentSpreadIdx - 1][0]);
      else goPrevChapter();
    } else {
    ...
    () =>
      mode === "long-strip"
        ...
        : mode === "single-rtl"
          ? goPrevPage()
          : goNextPage(),
    [mode, goNextChapter, goNextPage, goPrevPage],
  );
  ...
  const aggregateLoading =
    isMutating || (chapterAggregate && !chapterExists && !reachedMaxRetries);
  const aggregateError =
    (chapterAggregate && !chapterExists && reachedMaxRetries) || !!error;
        ...
        <LoadingNav
          button={
            ...
            >
              <Spinner />
              {retryCount > 0
                ? `Loading (${retryCount}/${MAX_RETRIES})`
                : "Loading..."}
            </Button>
          }
        />
        ...
        <LoadingNav
          button={
            <Button
              ...
              variant="destructive"
              onClick={handleAggregateRetry}
            >
              {error ? "Error. Retry?" : "No data. Retry?"}
            </Button>
          }
        />
      );
  ...
  return (
    ...
    >
      <CardContent className="flex gap-2 p-2 md:gap-1.5 md:p-1.5 w-full">
        <Button disabled size="icon" className="shrink-0 [&_svg]:size-5">
          <ArrowLeft />
        </Button>
        {button}
        <Button disabled size="icon" className="shrink-0 [&_svg]:size-5">
