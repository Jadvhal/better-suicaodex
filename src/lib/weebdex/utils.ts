 ...
 * - title: en > original title > first available alt title
...
export const parseMangaTitle = (
  ...
  // Prefer English title, then original title, then Japanese
  const enTitle = manga.alt_titles?.["en"]?.[0];
  const jaTitle = manga.alt_titles?.["ja"]?.[0];
  const title = enTitle ?? manga.title ?? jaTitle ?? "";
  ...
  const altTitlesRaw = Object.entries(manga.alt_titles ?? {})
    .filter(([lang]) => lang !== "en") // exclude primary language from alts
    .sort(([a], [b]) => langPriority(a) - langPriority(b))
    .flatMap(([, values]) => values);

  const altTitles =
    enTitle && manga.title && manga.title !== enTitle
      ? [manga.title, ...altTitlesRaw]
      : altTitlesRaw;
...
export const parseRelationTitle = (
  ...
  const enTitle = rel.alt_titles?.["en"]?.[0];
  const jaTitle = rel.alt_titles?.["ja"]?.[0];
  const title = enTitle ?? rel.title ?? jaTitle ?? "";
  ...
  const altTitles =
    enTitle && rel.title && rel.title !== enTitle
      ? [rel.title, ...altTitlesRaw]
      : altTitlesRaw;
...
export const generateJsonLd = (manga: Manga) => {
  ...
  const description =
    manga.description || `Read ${manga.title}`;
  return {
